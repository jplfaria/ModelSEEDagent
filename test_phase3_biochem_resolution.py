            ...
            try:
                ...
                if result.success:
                    ...
                    if expected.lower() in resolved_name.lower():
                        print(f"✅ {entity_id} → '{resolved_name}' (contains '{expected}')")
                    else:
                        print(f"❌ {entity_id} → '{resolved_name}' (expected '{expected}')")
                        all_passed = False
                else:
            ...
            try:
                ...
                if result.success:
                    ...
                    if result_count >= min_results:
                        print(f"✅ Search '{query}' → {result_count} results (≥{min_results})")
                        
                        # Show first result as example
                        if result.data["search_results"]:
                            first_result = result.data["search_results"][0]
                            print(f"   Example: {first_result['id']} → '{first_result['name']}'")
                    else:
                        print(f"❌ Search '{query}' → {result_count} results (expected ≥{min_results})")
                        all_passed = False
                else:
            ...
            try:
                ...
                if result.success:
                    ...
                    if cross_refs:
                        ...
                        print(f"⚠️  {entity_id} has no cross-references (may be expected)")
                else:
                    print(f"❌ {entity_id} → Failed: {result.message}")
                    all_passed = False
...
def test_edge_cases():
    ...
    try:
        ...
        test_cases = [
            ...
            # Invalid inputs
            ("resolver", {"entity_id": "nonexistent_12345"}, False, "nonexistent entity"),
            ("search", {"query": "xyzzylakjsdf"}, False, "nonsense query"),
            ("search", {"query": "ATP", "entity_type": "invalid"}, False, "invalid entity_type"),
            
            # Valid edge cases
            ("search", {"query": "ATP", "max_results": 1}, True, "limited results"),
            ("search", {"query": "ATP", "entity_type": "compound"}, True, "compound filter"),
            ("search", {"query": "kinase", "entity_type": "reaction"}, True, "reaction filter"),
        ]
            ...
            try:
                ...
                if should_succeed:
                    if result.success:
                        ...
                        print(f"❌ {description} → Failed unexpectedly: {result.message}")
                        all_passed = False
                else:
                    if not result.success:
                        print(f"✅ {description} → Failed as expected: {result.message}")
                    else:
