...
def test_langgraph_workflow():
    ...
    tools = [
        ...
        MockAnalysisTool({"name": "analyze_metabolic_model", "description": "Analysis tool"})
    ]
    ...
    with patch.object(llm, '_generate_response', side_effect=mock_generate_response):
        ...
        print(f"   Parallel capability: {'PASS' if len(tools_executed) >= 2 else 'FAIL'}")
        print(f"   Error recovery: {'PASS' if len(result.metadata.get('errors', [])) == 0 else 'FAIL'}")
        print(f"   State management: {'PASS' if metadata.get('final_status') == 'completed' else 'FAIL'}")
