...
class BaseSummarizer(ABC):
    ...
    def validate_size_limits(
        self, key_findings: List[str], summary_dict: Dict[str, Any]
    ) -> None:
        ...
        if key_findings_size > 2000:
            raise ValueError(
                f"key_findings too large: {key_findings_size}B > 2KB limit"
            )
    ...
    def __init__(self, base_dir: str = None):
        if base_dir is None:
            base_dir = os.environ.get(
                "MODELSEED_ARTIFACTS_DIR", "/tmp/modelseed_artifacts"
            )
        self.base_dir = Path(base_dir)
        self.base_dir.mkdir(parents=True, exist_ok=True)

    def store_artifact(
        self, data: Any, tool_name: str, model_id: str = "unknown", format: str = "json"
    ) -> str:
        ...
        if format == "json":
            ...
            if hasattr(data, "to_csv"):
                ...
                raise ValueError(
                    f"Data type {type(data)} not compatible with CSV format"
                )
        elif format == "pickle":
            import pickle
...
def enable_smart_summarization(
    ...
    if summarizer is None:
        ...
        if raw_data is not None:
            ...
            model_id = (
                model_stats.get("model_id", "unknown") if model_stats else "unknown"
            )
            artifact_path = artifact_storage.store_artifact(
                raw_data, tool_name, model_id
            )
