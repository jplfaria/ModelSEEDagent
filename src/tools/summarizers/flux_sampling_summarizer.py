...
class FluxSamplingSummarizer(BaseSummarizer):
    ...
    def _generate_key_findings(
        self, n_samples: int, n_reactions: int, analysis: Dict[str, Any], model_id: str
    ) -> List[str]:
        ...
        if flux_patterns:
            ...
            variable_pct = (
                (variable_count / n_reactions * 100) if n_reactions > 0 else 0
            )
        ...
        if correlations and "high_correlations" in correlations:
            ...
            if high_corr:
                ...
                key_findings.append(
                    f"High flux correlations found: {corr_count} reaction pairs"
                )
                ...
                key_findings.append(
                    f"Strongest correlation: {corr_val:.3f} between key reactions"
                )
        ...
        if dist_analysis and "objective_stats" in dist_analysis:
            ...
            if obj_stats and obj_stats.get("mean") is not None:
                ...
                key_findings.append(
                    f"Growth variability: mean={obj_mean:.3f}, CV={cv:.3f}"
                )
    ...
    def _generate_summary_dict(
        ...
        summary_dict = {
            "sampling_statistics": {
                ...
                "sampling_coverage": (
                    n_reactions / model_stats.get("num_reactions", n_reactions)
                    if model_stats
                    else 1.0
                ),
            },
        ...
        if flux_patterns:
            summary_dict["flux_pattern_summary"] = {
                ...
                "variable_reactions_count": len(
                    flux_patterns.get("variable_reactions", [])
                ),
        ...
        if dist_analysis:
            ...
            summary_dict["distribution_summary"] = {
                "objective_function": (
                    {
                        "mean": (
                            round(obj_stats.get("mean", 0), 4) if obj_stats else None
                        ),
                ...
                "sample_coverage": {
                    "reactions_sampled": dist_analysis.get(
                        "total_reactions_sampled", 0
                    ),
