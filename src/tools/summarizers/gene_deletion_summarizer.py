...
class GeneDeletionSummarizer(BaseSummarizer):
    ...
    def _generate_key_findings(
        ...
        if summary:
            ...
            essential_pct = (
                (essential_count / genes_tested * 100) if genes_tested > 0 else 0
            )
            impaired_pct = (
                (impaired_count / genes_tested * 100) if genes_tested > 0 else 0
            )
            no_effect_pct = (
                (no_effect_count / genes_tested * 100) if genes_tested > 0 else 0
            )
            ...
            if essentiality_rate > 0.15:
                key_findings.append(
                    "WARNING: High essentiality rate - model may be over-constrained"
                )
            elif essentiality_rate < 0.05:
                key_findings.append(
                    "WARNING: Low essentiality rate - check deletion thresholds"
                )
            else:
            ...
            if improved_count > 0:
                improved_pct = (
                    (improved_count / genes_tested * 100) if genes_tested > 0 else 0
                )
                key_findings.append(
                    f"Growth-improving deletions: {improved_count} ({improved_pct:.1f}%)"
                )
        ...
        if severely_impaired:
            key_findings.append(
                f"Severely impaired genes: {len(severely_impaired)} (growth 1-10%)"
            )
        if moderately_impaired:
            key_findings.append(
                f"Moderately impaired genes: {len(moderately_impaired)} (growth 10-50%)"
            )
        if mildly_impaired:
            key_findings.append(
                f"Mildly impaired genes: {len(mildly_impaired)} (growth 50-90%)"
            )
    ...
    def _generate_summary_dict(
        ...
        summary_dict = {
            "deletion_statistics": {
                ...
                "model_coverage": (
                    genes_tested / model_stats.get("num_genes", genes_tested)
                    if model_stats
                    else 1.0
                ),
            },
        ...
        if analysis:
            ...
            summary_dict["gene_categories"] = {
                "essential": {
                    ...
                    "percentage": (
                        (len(essential_genes) / genes_tested * 100)
                        if genes_tested > 0
                        else 0
                    ),
                ...
                "severely_impaired": {
                    ...
                    "percentage": (
                        (len(severely_impaired) / genes_tested * 100)
                        if genes_tested > 0
                        else 0
                    ),
                ...
                "moderately_impaired": {
                    ...
                    "percentage": (
                        (len(moderately_impaired) / genes_tested * 100)
                        if genes_tested > 0
                        else 0
                    ),
                },
                "mildly_impaired": {
                    ...
                    "percentage": (
                        (len(mildly_impaired) / genes_tested * 100)
                        if genes_tested > 0
                        else 0
                    ),
                },
                "no_effect": {
                    "count": len(no_effect),
                    "percentage": (
                        (len(no_effect) / genes_tested * 100) if genes_tested > 0 else 0
                    ),
                },
                "improved_growth": {
                    "count": len(improved_growth),
                    "percentage": (
                        (len(improved_growth) / genes_tested * 100)
                        if genes_tested > 0
                        else 0
                    ),
            ...
            summary_dict["essentiality_analysis"] = {
                ...
                "essentiality_category": self._categorize_essentiality_rate(
                    essentiality_rate
                ),
                "total_critical_genes": len(essential_genes) + len(severely_impaired),
                "robustness_score": (
                    (len(no_effect) / genes_tested) if genes_tested > 0 else 0
                ),
            }
        ...
        if deletion_results:
            growth_values = [
                result.get("growth_ratio", 0) for result in deletion_results.values()
            ]
            if growth_values:
                summary_dict["growth_impact_distribution"] = {
                    "mean_growth_retention": round(
                        sum(growth_values) / len(growth_values), 4
                    ),
