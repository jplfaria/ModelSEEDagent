...
class RastAnnotationTool(BaseTool):
        ...
        with open(clean_fasta_path, "w") as f:
            for feature in genome.features:
                ...
                # Only add ontology terms if they exist AND no function was found
                    ...
                    hasattr(feature, "ontology_terms")
                    and "RAST" in feature.ontology_terms
                ):
                    rast_terms = " ".join(
                        feature.ontology_terms["RAST"]
                    )  # Space instead of semicolon
                    header += f" {rast_terms}"
    ...
    def _run_tool(self, input_data: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if (
                input_data.get(
                    "generate_clean_fasta", self._annotation_config.generate_clean_fasta
                )
                or "output_fasta" in input_data
            ):
                output_path = input_data.get(
                    "output_fasta", f"{genome_name}_annotated.faa"
                )
            ...
            stats = {
                ...
                "num_features": (
                    len(genome.features) if hasattr(genome, "features") else 0
                ),
                "num_annotated": (
                    len(
                        [
                            ...
                            if hasattr(f, "function") and f.function
                        ]
                    )
                ...
                "annotation_jobs": (
                    annotation_result if isinstance(annotation_result, list) else []
                ),
            }

            # Calculate annotation success rate
            success_rate = (
                (stats["num_annotated"] / stats["num_features"])
