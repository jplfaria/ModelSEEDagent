...
class AuxotrophyConfig(BaseModel):
    ...
    candidate_metabolites: list = Field(
        default_factory=lambda: ["EX_arg__L_e", "EX_leu__L_e", "EX_lys__L_e"]
    )
    ...
    def _run_tool(self, input_data: Any) -> ToolResult:
        try:
            ...
            return ToolResult(
                ...
                data={
                    "complete_growth_rate": complete_growth,
            ...
            return ToolResult(
                success=False, message="Error identifying auxotrophies.", error=str(e)
            )
