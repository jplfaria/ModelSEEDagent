...
class BiochemEntityResolverInput(BaseModel):
    ...
    include_aliases: bool = Field(
        default=True,
        description="Whether to include alias mappings in the results"
    )
...
class BiochemEntityResolverOutput(BaseModel):
    ...
    suggestions: List[str] = Field(description="Suggested similar entities if not resolved")
...
class BiochemEntityResolverTool(BaseTool):
    ...
    def __init__(self):
        super().__init__(
            ...
            description="Resolve biochemistry entity IDs (compounds/reactions) to human-readable names and aliases",
            input_schema=BiochemEntityResolverInput,
            output_schema=BiochemEntityResolverOutput
        )
        self._db_path = None
        ...
        if self._db_path is None:
            ...
            if not db_path.exists():
                raise FileNotFoundError(
                    ...
                    "Please run scripts/build_mvp_biochem_db.py to create it."
                )
    ...
    def _resolve_compound(self, conn: sqlite3.Connection, entity_id: str, 
                         include_aliases: bool, max_results: int) -> Dict[str, Any]:
        ...
        if result:
            return self._build_compound_result(conn, result[0], result[1], include_aliases)
        ...
        if results:
            ...
            return self._build_compound_result(conn, results[0][0], results[0][1], include_aliases)
    ...
    def _resolve_reaction(self, conn: sqlite3.Connection, entity_id: str,
                         include_aliases: bool, max_results: int) -> Dict[str, Any]:
        ...
        result = cursor.fetchone()
        if result:
            return self._build_reaction_result(conn, result[0], result[1], include_aliases)
        ...
        results = cursor.fetchall()
        if results:
            return self._build_reaction_result(conn, results[0][0], results[0][1], include_aliases)
    ...
    def _build_compound_result(self, conn: sqlite3.Connection, modelseed_id: str, 
                              primary_name: str, include_aliases: bool) -> Dict[str, Any]:
        ...
        if include_aliases:
            ...
            result['aliases'] = [
                {'external_id': row[0], 'source': row[1]}
    ...
    def _build_reaction_result(self, conn: sqlite3.Connection, modelseed_id: str,
                              primary_name: str, include_aliases: bool) -> Dict[str, Any]:
    ...
    def _get_compound_suggestions(self, conn: sqlite3.Connection, entity_id: str) -> List[str]:
    ...
    def _get_reaction_suggestions(self, conn: sqlite3.Connection, entity_id: str) -> List[str]:
    ...
    def run(self, input_data: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            entity_type = input_obj.entity_type or self._detect_entity_type(input_obj.entity_id)
            ...
            try:
                ...
                if entity_type == 'compound':
                    resolution = self._resolve_compound(
                        conn, input_obj.entity_id, 
                        input_obj.include_aliases, input_obj.max_results
                    )
...
class BiochemSearchTool(BaseTool):
    ...
    def __init__(self):
        super().__init__(
            name="search_biochem",
            description="Search the biochemistry database for compounds and reactions by name or alias",
            input_schema=BiochemSearchInput,
            output_schema=BiochemSearchOutput
        )
        self._db_path = None
    ...
    def _search_compounds(self, conn: sqlite3.Connection, query: str, 
        ...
        if source_filter:
            sql = """
                ...
                LIMIT ?
            """
            params = (f"%{query}%", source_filter, f"%{query}%", source_filter, max_results)
        else:
            sql = """
                SELECT DISTINCT c.modelseed_id, c.primary_name,
                       cn.name as match_name, cn.source, 'name' as match_type
    ...
    def _search_reactions(self, conn: sqlite3.Connection, query: str,
                         source_filter: Optional[str], max_results: int) -> List[Dict[str, Any]]:
            ...
            try:
                ...
                if input_obj.entity_type == 'compound':
                    compounds = self._search_compounds(
                        conn, input_obj.query, input_obj.source_filter, input_obj.max_results
                    )
            ...
            if isinstance(obj, dict):
                for key, value in obj.items():
                    if isinstance(value, str) and (
                        ...
                        "reaction" in key.lower() or "compound" in key.lower()
                    ):
