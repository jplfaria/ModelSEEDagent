...
class AgentState(TypedDict):
    ...
    run_id: str
    timestamp: str
    workflow_state: Literal["thinking", "planning", "executing", "parallel_exec", "analyzing", "completed", "error"]
...
class EnhancedVectorStore:
    ...
    def query(self, query_text: str, top_n: int = 3, min_similarity: float = 0.1) -> List[Dict[str, Any]]:
...
class SimulationResultsStore:
    ...
    def save_results(self, tool_name: str, model_id: str, solution, metadata: Dict[str, Any] = None) -> str:
        ...
        result_data = {
            ...
            "model_id": model_id,
            "timestamp": timestamp,
            "objective_value": float(solution.objective_value) if hasattr(solution, 'objective_value') else None,
            "status": str(solution.status) if hasattr(solution, 'status') else "unknown",
    ...
    def _export_result_files(self, result_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        if result_data.get("fluxes"):
            try:
                import pandas as pd
                flux_df = pd.DataFrame.from_dict(result_data["fluxes"], orient='index', columns=['flux'])
...
class LangGraphMetabolicAgent(BaseAgent):
    ...
    def _planning_node(self, state: AgentState) -> AgentState:
        ...
        if state["iteration"] >= state["max_iterations"]:
            ...
            state["final_answer"] = f"Reached maximum iterations ({state['max_iterations']}). " + \
    ...
    def _should_continue(self, state: AgentState) -> str:
        ...
        if next_action in ["single_tool", "parallel_tools", "analyze", "finalize", "error"]:
            return next_action
    ...
    def _analyze_results(self, state: AgentState) -> str:
        ...
        if state.get("continue_analysis", False) and state["iteration"] < state["max_iterations"]:
            state["iteration"] += 1
            return "continue"
...
Provide a concise analysis and indicate if you need to continue or can finalize."""
    ...
    def _should_continue_analysis(self, analysis: str, state: AgentState) -> bool:
        ...
        continue_keywords = ["need more", "additional", "further analysis", "not sufficient", "incomplete"]
        
        # Keywords that suggest completion
        complete_keywords = ["sufficient", "complete", "ready to answer", "have enough", "can conclude"]
        
        continue_score = sum(1 for keyword in continue_keywords if keyword in analysis_lower)
        complete_score = sum(1 for keyword in complete_keywords if keyword in analysis_lower)
    ...
    def _simplify_query(self, query: str) -> str:
        ...
        simplified = re.sub(r'\b(detailed|comprehensive|complex|advanced)\b', 'basic', query.lower())
        return simplified
    ...
    def analyze_model(self, query: str, model_path: Optional[str] = None) -> AgentResult:
