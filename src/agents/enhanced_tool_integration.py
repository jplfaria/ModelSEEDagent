...
class EnhancedToolIntegration:
    ...
    def analyze_query_intent(self, query: str) -> Dict[str, Any]:
        ...
        intent_patterns = {
            "structural_analysis": [
                "structure", "network", "connectivity", "topology", 
                "reactions", "metabolites", "genes"
            ],
            "growth_analysis": [
                "growth", "fba", "flux", "biomass", "objective"
            ],
            "pathway_analysis": [
                "pathway", "subsystem", "central carbon", "glycolysis"
            ],
            "optimization": [
                "optimize", "minimal", "media", "improve", "design"
            ],
            "validation": [
                "validate", "check", "missing", "auxotrophy", "gaps"
            ],
            "comprehensive": [
        ...
        if not intent_scores:
            ...
            intent_analysis["secondary_intents"] = [intent for intent, _ in sorted_intents[1:]]
        ...
        intent_tool_mapping = {
            ...
            "comprehensive": ["analyze_metabolic_model", "run_metabolic_fba", "analyze_pathway"]
        }
        
        # Build suggested tools list
        suggested_tools = set()
        for intent in [intent_analysis["primary_intent"]] + intent_analysis["secondary_intents"]:
        ...
        logger.info(f"Query intent analysis: {intent_analysis['primary_intent']} with {len(suggested_tools)} tools")
        return intent_analysis
    
    def create_execution_plan(self, intent_analysis: Dict[str, Any], model_path: str = None) -> List[ToolExecutionPlan]:
        """
        ...
        for i, tool_name in enumerate(suggested_tools):
            ...
            parallel_group = None
            if complexity == "complex" and category in [ToolCategory.FUNCTIONAL, ToolCategory.VALIDATION]:
    ...
    def execute_tool_with_monitoring(self, plan: ToolExecutionPlan) -> ToolExecutionResult:
        """
    ...
    def _check_performance(self, exec_result: ToolExecutionResult):
        ...
        if tool_name in self.performance_baselines:
            ...
            if execution_time > max_time:
                logger.warning(f"Tool {tool_name} exceeded max time: {execution_time:.2f}s > {max_time}s")
            elif execution_time > avg_time * 2:
                logger.info(f"Tool {tool_name} slower than expected: {execution_time:.2f}s > {avg_time * 2}s")
    
    def analyze_workflow_dependencies(self, execution_plan: List[ToolExecutionPlan]) -> Dict[str, Any]:
        """
    ...
    def _find_critical_path(self, dependency_graph: Dict[str, List[str]], 
                           execution_plan: List[ToolExecutionPlan]) -> List[str]:
    ...
    def create_workflow_visualization(self, execution_plan: List[ToolExecutionPlan], 
                                    execution_results: List[ToolExecutionResult] = None) -> str:
        """
        ...
        node_trace = go.Scatter(
            ...
            customdata=[[G.nodes[node].get('category', 'unknown'),
                        ...
                        G.nodes[node].get('execution_time', 0) or 0] for node in G.nodes()]
        )
    ...
    def create_performance_dashboard(self) -> str:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Execution Times', 'Success Rate', 'Tool Categories', 'Performance Trends'),
        ...
        fig.add_trace(
            go.Bar(x=list(categories.keys()), y=list(categories.values()), name='Category Usage'),
            row=2, col=1
        )
    ...
    def _generate_performance_insights(self) -> List[str]:
        ...
        # Find slowest tool
        slowest_tool = max(tool_performance.items(), 
                          key=lambda x: sum(x[1]["times"]) / len(x[1]["times"]))
        insights.append(f"Slowest tool: {slowest_tool[0]} (avg: {sum(slowest_tool[1]['times'])/len(slowest_tool[1]['times']):.2f}s)")
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if tool_usage:
            ...
            recommendations.append(f"Consider caching results for {most_used[0]} (used {most_used[1]} times)")
        
        # Performance recommendations
        avg_time = sum(r.execution_time for r in self.execution_history) / len(self.execution_history)
        if avg_time > 5:
            recommendations.append("Consider implementing parallel execution for independent tools")
        ...
        if failed_tools:
            ...
            recommendations.append(f"Investigate and improve reliability of {most_failed}")
