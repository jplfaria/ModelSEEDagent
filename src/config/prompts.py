...
class PromptManager:
    ...
    def load_prompts(cls, prompts_dir: Optional[Union[str, Path]] = None) -> Prompts:
        ...
        try:
            ...
            if (
                "prefix" in agent_config
                and "format_instructions" in agent_config
                and "suffix" in agent_config
            ):
    ...
    try:
        ...
        if template_name in prompts.templates:
            ...
            return LangChainPromptTemplate(
                template=template.template, input_variables=template.input_variables
            )
...
def get_prompt(
    agent_type: str = None, tool_name: str = None, template_name: str = None
) -> Union[AgentPrompts, ToolPrompts, PromptTemplate]:
    if agent_type:
