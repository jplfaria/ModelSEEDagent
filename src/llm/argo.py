...
class ArgoLLM(BaseLLM):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._debug = config.get('debug', False) or os.getenv('ARGO_DEBUG', '').lower() in ('1', 'true')
        ...
        if self._is_test:
            ...
            self._url = self._base_url + ('streamchat/' if self._use_streaming else 'chat/')
        ...
        logger.info(
            ...
            self._model_name, self._env, self._timeout, self._url
        )
    ...
    def _test_dual_env_connectivity(self):
        ...
        if (self._env == 'prod' and 
            self._model_name in DUAL_ENV_MODELS and 
            not self._ping_environment()):
    ...
    def _generate_response(self, 
        ...
        if stop_sequences and not self._model_name.startswith('o'):
    ...
    def _generate_response_advanced(self, payload: Dict[str, Any]) -> LLMResponse:
        ...
        for attempt in range(self._retries + 1):
            ...
            try:
                ...
                logger.debug(f"Status {response.status_code} | preview: {response.text[:120]}")
                ...
                if response.status_code in PROCESSING_CODES:
                    ...
                    logger.warning(f"5xx ({response.status_code}) on attempt {attempt + 1}")
                    
                    if (self._model_name in DUAL_ENV_MODELS and 
                        self._env == 'prod' and 
                        attempt == 0):
                    ...
                    if text:
                        tokens_used = self._estimate_tokens_from_response(response, text)
                        self.update_usage(tokens_used)
                        ...
                        if not sentinel_injected:
                            ...
                            original_prompt = payload.get('prompt', [''])[0] if 'prompt' in payload else payload.get('messages', [{}])[-1].get('content', '')
                            sentinel_prompt = f"Response: {original_prompt}"
                            payload = self._build_payload(sentinel_prompt, self.config.system_content or "")
            ...
            if attempt < self._retries:
                ...
                logger.info(f"[retry {attempt+1}/{self._retries}] {reason}; sleeping {delay:.1f}s")
                time.sleep(delay)
    ...
    def _poll_for_result(self, initial_response: httpx.Response) -> Optional[LLMResponse]:
    ...
    def _estimate_tokens_from_response(self, response: httpx.Response, text: str) -> int:
