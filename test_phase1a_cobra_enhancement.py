...
def test_new_cobra_tools():
    ...
    try:
        ...
        if result.success:
            print(f"  ✅ FVA completed for {result.data['summary']['total_reactions']} reactions")
            print(f"  📊 {result.data['summary']['variable_reactions']} variable reactions found")
            tests_passed += 1
        else:
    ...
    try:
        ...
        if result.success:
            ...
            print(f"  📊 Essential genes: {result.data['analysis']['summary']['essential_count']}")
            tests_passed += 1
        else:
    ...
    try:
        ...
        if result.success:
            ...
            if result.data["essential_reactions"]:
                print(f"  📊 Essential reactions: {len(result.data['essential_reactions'])}")
            tests_passed += 1
        else:
    ...
    try:
        ...
        if result.success:
            print(f"  ✅ Flux sampling completed with {result.metadata['n_samples']} samples")
    ...
    if tests_passed == total_tests:
        ...
        print("  ✅ EssentialityAnalysisTool - Essential component identification added")
        ...
        print(f"❌ {total_tests - tests_passed} tests failed. Please address issues before proceeding.")
        return False
