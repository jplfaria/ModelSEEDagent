...
def test_live_layout_updates():
    ...
    try:
        ...
        layout["right"].split_column(Layout(name="tools", size=6), Layout(name="results"))
        
        # Initialize with proper content
        layout["header"].update(Panel("Test Query Analysis", style="bold blue"))
        layout["thinking"].update(Panel("[dim]Waiting for AI...[/dim]", title="AI Thinking"))
        layout["decisions"].update(Panel("[dim]No decisions yet...[/dim]", title="Decisions"))
        ...
        with Live(layout, console=console, refresh_per_second=4) as live:
            ...
            for i in range(8):
                ...
                layout["thinking"].update(Panel(thinking_content, title="AI Thinking", border_style="cyan"))
                
                # Update decisions panel every 2 steps
                if i % 2 == 0:
                    decisions_content = f"Decision {i//2 + 1}: Proceed with analysis"
                    layout["decisions"].update(Panel(decisions_content, title="Decisions", border_style="yellow"))
                
                # Update tools panel every 3 steps
                if i % 3 == 0:
                    tools_content = f"Tool {i//3 + 1}: Executing analysis..."
                    layout["tools"].update(Panel(tools_content, title="Tools", border_style="green"))
                
                # Update results panel
                results_content = f"Progress: {i+1}/8 steps completed"
                layout["results"].update(Panel(results_content, title="Results", border_style="magenta"))
...
def test_fixed_content_validation():
    ...
    try:
        ...
        layout["header"].update(Panel("Testing Fixed Content Validation", style="bold green"))
        ...
        with Live(layout, console=console, refresh_per_second=4) as live:
            for i in range(10):
                ...
                decisions_panel = safe_create_panel(f"Decision {i}", "Decisions", "yellow")
