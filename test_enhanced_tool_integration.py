    ...
    def _run(self, input_data):
        return ToolResult(
            ...
            data={
                ...
                "cofactor_usage": {"ATP": 15.6, "NADH": 8.9, "NADPH": 4.2},
            },
        )
...
def test_enhanced_tool_integration():
    ...
    tools = [
        EnhancedMockFBATool(
            {"name": "run_metabolic_fba", "description": "Enhanced FBA tool"}
        ),
        EnhancedMockAnalysisTool(
            {"name": "analyze_metabolic_model", "description": "Enhanced analysis tool"}
        ),
        EnhancedMockPathwayTool(
            {"name": "analyze_pathway", "description": "Enhanced pathway tool"}
        ),
    ]
    ...
    execution_plan = tool_integration.create_execution_plan(
        intent_analysis, "e_coli_core.xml"
    )

    print(f"   Created execution plan with {len(execution_plan)} tools:")
    for plan in execution_plan:
        print(
            f"   - {plan.tool_name} (Priority: {plan.priority.name}, Category: {plan.category.value})"
        )
        if plan.depends_on:
    ...
    print(
        f"   Average execution time: {execution_summary['average_execution_time']:.3f}s"
    )
    ...
    try:
        ...
        viz_path = tool_integration.create_workflow_visualization(
            execution_plan, tool_integration.execution_history
        )
        ...
        print(
            f"   ⚠️ Visualization creation failed (may need additional dependencies): {e}"
        )
...
def test_enhanced_langgraph_agent():
    ...
    with patch.object(llm, "_generate_response", side_effect=mock_generate_response):
        ...
        print(
            f"   Workflow complexity: {result.metadata.get('workflow_complexity', 'unknown')}"
        )
        print(
            f"   Total execution time: {result.metadata.get('total_execution_time', 0):.3f}s"
        )
        print(
            f"   Visualization count: {result.metadata.get('visualization_count', 0)}"
        )
        ...
        if intent_analysis:
            print(
                f"   Intent analysis: {intent_analysis.get('primary_intent', 'unknown')}"
            )
        ...
        if performance_metrics:
            print(
                f"   Performance metrics available for {len(performance_metrics)} tools"
            )
