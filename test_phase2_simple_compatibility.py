...
def test_sbml_roundtrip_compatibility():
    ...
    try:
        ...
        print(f"✅ Loaded original model: {len(original_model.reactions)} reactions, {len(original_model.metabolites)} metabolites")
        ...
        with reimported_model:
            ...
            reimported_growth = reimported_solution.objective_value if reimported_solution.status == 'optimal' else 0
        
        print(f"   Re-imported growth rate: {reimported_growth:.6f}")
        
        # Compare structures
        print(f"   Structure comparison:")
        print(f"     Reactions: {len(original_model.reactions)} → {len(reimported_model.reactions)}")
        print(f"     Metabolites: {len(original_model.metabolites)} → {len(reimported_model.metabolites)}")
        print(f"     Genes: {len(original_model.genes)} → {len(reimported_model.genes)}")
        ...
        print(f"   Growth compatible (tolerance {tolerance}): {'✅ YES' if growth_compatible else '❌ NO'}")
...
def test_modelseed_cobra_integration():
    ...
    try:
        ...
        with model:
            ...
            print(f"     FBA: {'✅ PASS' if fba_works else '❌ FAIL'} (growth: {fba_solution.objective_value:.6f})")
        ...
        try:
            ...
            test_genes = list(model.genes)[:3] if len(model.genes) > 3 else list(model.genes)
            ...
            print(f"     Gene Deletion: {'✅ PASS' if gene_deletion_works else '❌ FAIL'}")
        except Exception as e:
        ...
        print(f"   Integration Success: {'✅ PASS' if integration_success else '❌ PARTIAL'} ({sum(all_tests)}/{len(all_tests)} tools)")
...
def test_compatibility_metrics():
    ...
    try:
        ...
        # Structure metrics
        metrics['reactions_identical'] = len(original_model.reactions) == len(converted_model.reactions)
        metrics['metabolites_identical'] = len(original_model.metabolites) == len(converted_model.metabolites)
        metrics['genes_identical'] = len(original_model.genes) == len(converted_model.genes)
        ...
        print(f"     Structure - Reactions: {'✅' if metrics['reactions_identical'] else '❌'}")
        print(f"     Structure - Metabolites: {'✅' if metrics['metabolites_identical'] else '❌'}")
        print(f"     Structure - Genes: {'✅' if metrics['genes_identical'] else '❌'}")
        print(f"     Growth Rate Compatible: {'✅' if metrics['growth_compatible'] else '❌'}")
        print(f"     Growth Difference: {metrics['growth_difference']:.8f}")
        print(f"     Growth Relative Diff: {metrics['growth_relative_diff_percent']:.6f}%")
