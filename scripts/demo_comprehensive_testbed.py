...
# Import tools
from src.tools.cobra import FBATool, EssentialityAnalysisTool
from src.tools.cobra.media_tools import MediaSelectorTool, MediaCompatibilityTool
...
def demo_biological_validation():
    ...
    for tool_name, tool in tools_to_test.items():
        ...
        try:
            ...
            if tool_name == "FBA":
                ...
                params = {"entity_id": "glc__D", "entity_type": "compound", "include_aliases": True}
    ...
    print(f"  Total Time: {sum(r.get('execution_time', 0) for r in results.values()):.2f}s")
    ...
    output_file = Path("testbed_results") / f"demo_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def validate_result(tool_name: str, output: dict) -> dict:
    ...
    if tool_name == "FBA":
        ...
        if 0.1 <= growth_rate <= 1.5:
            validation["biological_insights"].append(f"Growth rate {growth_rate:.3f} h⁻¹ is biologically realistic")
        ...
        if total_genes > 0:
            ...
            if 10 <= essential_percent <= 20:
                validation["biological_insights"].append(f"Essential gene percentage {essential_percent:.1f}% is typical")
                validation["scores"]["essentiality"] = 1.0
            else:
                validation["warnings"].append(f"Unusual essential gene percentage: {essential_percent:.1f}%")
                
    elif tool_name == "MediaSelector":
        best_media = data.get("best_media")
        if best_media:
            validation["biological_insights"].append(f"Selected {best_media} as optimal media")
            validation["scores"]["media_selection"] = 1.0
            
    elif tool_name == "MediaCompatibility":
        compatibility_results = data.get("compatibility_results", {})
        if compatibility_results:
            avg_compatibility = sum(r.get("compatibility_score", 0) for r in compatibility_results.values()) / len(compatibility_results)
            if avg_compatibility > 0.7:
                validation["biological_insights"].append(f"High media compatibility: {avg_compatibility:.2f}")
                validation["scores"]["compatibility"] = avg_compatibility
            else:
                validation["warnings"].append(f"Low media compatibility: {avg_compatibility:.2f}")
                
    elif tool_name == "BiochemEntityResolver":
        resolved = data.get("resolved", False)
        primary_name = data.get("primary_name", "")
        if resolved and primary_name:
            validation["biological_insights"].append(f"Successfully resolved to: {primary_name}")
