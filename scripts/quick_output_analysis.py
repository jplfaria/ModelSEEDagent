...
# Import tools
from src.tools.cobra import FBATool, EssentialityAnalysisTool, FluxVariabilityTool, FluxSamplingTool
...
def analyze_output_sizes():
    ...
    tools_to_test = {
        ...
        "FluxVariability": (FluxVariabilityTool(basic_config), {"model_path": model_path, "fraction_of_optimum": 0.9}),
        "FluxSampling": (FluxSamplingTool(basic_config), {"model_path": model_path, "n_samples": 100, "method": "optgp"}),
        "Essentiality": (EssentialityAnalysisTool(basic_config), {"model_path": model_path, "threshold": 0.01}),
        "MediaSelector": (MediaSelectorTool(basic_config), {"model_path": model_path, "target_growth": 0.1}),
        "BiochemEntityResolver": (BiochemEntityResolverTool(basic_config), {"entity_id": "glc__D", "entity_type": "compound", "include_aliases": True}),
    }
    ...
    for tool_name, (tool, params) in tools_to_test.items():
        ...
        try:
            ...
            results[tool_name] = {
                ...
                "requires_summarization": size_kb > 50,  # > 50KB likely too large for LLM
            ...
            print(f"  {status} {tool_name}: {execution_time:.2f}s | {size_kb:.1f} KB {size_flag}")
    ...
    large_outputs = [name for name, result in results.items() 
    ...
    medium_outputs = [name for name, result in results.items() 
                     if result.get("size_kb", 0) > 10 and not result.get("requires_summarization", False)]
    
    if medium_outputs:
        print(f"🟡 Tools with medium outputs (10-50KB): {', '.join(medium_outputs)}")
        print("   These tools might benefit from summarization")
    
    small_outputs = [name for name, result in results.items() 
    ...
    # Save results
    output_file = Path("testbed_results") / f"output_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
