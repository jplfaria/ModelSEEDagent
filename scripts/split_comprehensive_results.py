...
class ComprehensiveResultsAnalyzer:
    ...
    def __init__(self):
        self.tool_categories = {
            "COBRA": ["FBA", "ModelAnalysis", "FluxVariability", 
                     "GeneDeletion", "Essentiality", "FluxSampling", "ProductionEnvelope",
                     "Auxotrophy", "MinimalMedia", "MissingMedia", "ReactionExpression"],
            "AI_Media": ["MediaSelector", "MediaManipulator", "MediaCompatibility", 
                        "MediaComparator", "MediaOptimization", "AuxotrophyPrediction"],
    ...
    def split_comprehensive_results(self, input_file_path: str):
        ...
        print(f"📊 Processing {len(models_tested)} models × {metadata['tools_tested']['total_count']} tools")
        print(f"🔬 Including biological validation analysis")
        print("=" * 70)

        # Split individual results
        individual_files = self._split_individual_results(data, individual_dir, models_tested)
    ...
    def _split_individual_results(self, data: Dict, individual_dir: Path, models_tested: List[str]) -> List[str]:
            ...
            for tool_name, tool_result in model_results.items():
                ...
                individual_result = {
                    "metadata": {
                        ...
                        "has_biological_validation": "biological_validation" in tool_result,
                    },
                    "test_parameters": {
                    ...
                    "biological_validation": tool_result.get("biological_validation", {}),
                    "debug_info": tool_result.get("debug_info", {}),
                }
                
                # Add category-specific analysis
                if tool_category == "COBRA":
                    individual_result["cobra_analysis"] = self._analyze_cobra_result(tool_result)
                elif tool_category == "AI_Media":
                    individual_result["media_analysis"] = self._analyze_media_result(tool_result)
                elif tool_category == "Biochemistry":
                    individual_result["biochem_analysis"] = self._analyze_biochem_result(tool_result)
                ...
                print(f"  {status} {tool_name:<22} {time_str:>8} {bio_indicator:>10} → {output_filename} ({file_size:.1f} KB)")
    ...
    def _analyze_cobra_result(self, tool_result: Dict) -> Dict[str, Any]:
        ...
        if isinstance(output, dict):
            ...
            if data and "significant_fluxes" in data:
                analysis["metrics"]["active_reactions"] = len(data["significant_fluxes"])
    ...
    def _generate_comprehensive_analyses(self, data: Dict, analysis_dir: Path) -> Dict[str, str]:
        ...
        for analysis_name, file_path in analyses.items():
            ...
            print(f"  📈 {analysis_name:<25} → {os.path.basename(file_path)} ({file_size:.1f} KB)")
    ...
    def _analyze_biological_validation(self, data: Dict) -> Dict[str, Any]:
        ...
        for model_name, model_results in data["results"].items():
            ...
            analysis["by_model"][model_name] = {
                "avg_bio_score": statistics.mean(model_bio_scores) if model_bio_scores else 0,
                "bio_score_count": len(model_bio_scores),
                "score_range": [min(model_bio_scores), max(model_bio_scores)] if model_bio_scores else [0, 0]
            }
        
        # Overall statistics
        if all_bio_scores:
            analysis["summary"]["avg_bio_score"] = statistics.mean(all_bio_scores)
            analysis["summary"]["bio_score_std"] = statistics.stdev(all_bio_scores) if len(all_bio_scores) > 1 else 0
            analysis["summary"]["bio_score_range"] = [min(all_bio_scores), max(all_bio_scores)]
    ...
    def _analyze_cross_model_performance(self, data: Dict) -> Dict[str, Any]:
        ...
        for model_name in data["results"].keys():
            if model_name in self.model_types["BiGG"]:
                analysis["format_specific_performance"]["BiGG"]["models"].append(model_name)
            elif model_name in self.model_types["ModelSEED"]:
                analysis["format_specific_performance"]["ModelSEED"]["models"].append(model_name)
        ...
        for tool_name in all_tools:
            ...
            for model_name, model_results in data["results"].items():
                if tool_name in model_results:
                    tool_results[model_name] = model_results[tool_name].get("success", False)
            
            analysis["tool_consistency"][tool_name] = {
                "tested_on_models": list(tool_results.keys()),
                "success_by_model": tool_results,
                "overall_success_rate": sum(tool_results.values()) / len(tool_results) if tool_results else 0,
                "consistency_score": len(set(tool_results.values())) == 1  # All same result
            }
    ...
    def _analyze_tool_categories(self, data: Dict) -> Dict[str, Any]:
        ...
        for category, tool_names in self.tool_categories.items():
            ...
            if category_results["execution_times"]:
                category_results["avg_execution_time"] = statistics.mean(category_results["execution_times"])
                category_results["execution_time_std"] = statistics.stdev(category_results["execution_times"]) if len(category_results["execution_times"]) > 1 else 0
            
            if category_results["bio_scores"]:
                category_results["avg_bio_score"] = statistics.mean(category_results["bio_scores"])
            
            category_results["success_rate"] = category_results["successful_tests"] / category_results["total_tests"] if category_results["total_tests"] > 0 else 0
        ...
        for tool_name in media_tools:
            ...
            for model_name, model_results in data["results"].items():
                if tool_name in model_results:
                    ...
                    model_format = "BiGG" if model_name in self.model_types["BiGG"] else "ModelSEED"
    ...
    def _create_comprehensive_summary(self, data: Dict, individual_files: List[str], analyses: Dict[str, str]) -> Dict[str, Any]:
        ...
        for model_name in metadata["models_tested"]:
            if model_name in data["results"]:
                model_results = data["results"][model_name]
                model_success = sum(1 for r in model_results.values() if r.get("success", False))
        ...
        summary = {
            ...
            "test_summary": {
                ...
                "biological_validation_summary": metadata.get("biological_validation_summary", {})
            },
            ...
            "quality_metrics": {
                ...
                "biological_validation_coverage": self._calculate_bio_validation_coverage(data),
    ...
    def _calculate_category_coverage(self, data: Dict) -> Dict[str, float]:
        ...
        for category, tools in self.tool_categories.items():
            ...
            category_coverage[category] = category_success / category_total if category_total > 0 else 0
...
def main():
    ...
    try:
        ...
        individual_files, analyses, summary_file = analyzer.split_comprehensive_results(str(latest_file))
