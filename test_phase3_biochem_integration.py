...
from tools.biochem.resolver import BiochemEntityResolverTool, BiochemSearchTool
from tools.biochem.resolver import resolve_entity_id, enhance_tool_output_with_names
        ...
        try:
            ...
            if result.success:
                ...
                if resolved == expected_resolved and entity_type == expected_type:
                    ...
                    print(f"❌ {entity_id}: Expected resolved={expected_resolved}, type={expected_type}, "
                ...
                if not expected_resolved:
                    ...
                    print(f"❌ {entity_id}: Expected to resolve but failed: {result.error}")
        ...
        try:
            ...
            if result.success:
                ...
                if total_results >= min_expected:
                    print(f"✅ '{query}': Found {total_results} results "
                          f"({len(compounds)} compounds, {len(reactions)} reactions)")
                    
                    # Show top results
                    if compounds:
                        print(f"   Top compound: {compounds[0]['primary_name']} ({compounds[0]['modelseed_id']})")
                    if reactions:
                        print(f"   Top reaction: {reactions[0]['primary_name']} ({reactions[0]['modelseed_id']})")
                    
                    success_count += 1
                else:
                    print(f"❌ '{query}': Expected ≥{min_expected} results, got {total_results}")
            else:
                if min_expected == 0:
...
def test_database_performance():
    ...
    print(f"✅ Resolved {resolved_count}/{len(test_entities)} entities in {duration:.3f} seconds")
    ...
    if result.success:
        ...
        print(f"✅ Search for 'glucose' found {total_results} results in {end_time - start_time:.3f} seconds")
    else:
    ...
    for alias, expected_sources in alias_tests:
        try:
            ...
            if result.success and result.data.get("resolved"):
                ...
                expected_sources_found = any(source in sources_found for source in expected_sources)
                
                if expected_sources_found:
                    print(f"✅ {alias}: Found aliases from {len(sources_found)} sources")
                    print(f"   Sources: {', '.join(sorted(sources_found)[:5])}...")
                    success_count += 1
                else:
                    print(f"❌ {alias}: Expected sources {expected_sources}, found {sources_found}")
            else:
