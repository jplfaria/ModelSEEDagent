...
def test_live_display_with_fixed_panels():
    ...
    try:
        ...
        layout["header"].update(
            Panel("Fixed Panel Test - No Flickering!", style="bold green")
        )
        ...
        with Live(layout, console=console, refresh_per_second=4) as live:
            ...
            for i, (thought, decisions) in enumerate(test_sequence):
                ...
                info_panel = safe_create_panel(
                    f"Test {i+1} info", "Information", "blue"
                )
...
def test_streaming_demo():
    ...
    try:
        ...
        layout["header"].update(
            Panel("🧬 ModelSEEDagent Analysis Demo", style="bold cyan")
        )

        # Initialize with safe content
        layout["thinking"].update(
            safe_create_panel("Initializing AI agent...", "AI Thinking", "cyan")
        )
        layout["decisions"].update(
            safe_create_panel("Preparing analysis...", "AI Decisions", "yellow")
        )
        layout["tools"].update(
            safe_create_panel("Loading tools...", "Tool Execution", "green")
        )
        layout["results"].update(
            safe_create_panel("Starting up...", "Live Results", "magenta")
        )
        ...
        with Live(layout, console=console, refresh_per_second=4) as live:
            workflow = [
                ...
                (
                    "🧠 Query parsed - planning analysis strategy",
                    "🎯 Decision: Start with growth analysis",
                ),
                (
                    "🧠 Selecting optimal first tool...",
                    "🎯 Decision: Use FBA for baseline",
                ),
                (
                    "⚡ Executing run_metabolic_fba...",
                    "🎯 FBA will show growth capacity",
                ),
                (
                    "🧠 FBA shows 0.518 h⁻¹ growth rate",
                    "🎯 High rate - check requirements",
                ),
                (
                    "⚡ Executing find_minimal_media...",
                    "🎯 Need to find nutrient dependencies",
                ),
                ("🧠 Found 20 essential nutrients", "🎯 Moderate complexity detected"),
                (
                    "⚡ Executing analyze_essentiality...",
                    "🎯 Complete characterization needed",
                ),
                (
                    "🧠 142 essential genes identified",
                    "🎯 Analysis strategy successful",
                ),
            ...
            for i, (thinking, decision) in enumerate(workflow):
                # Update thinking
                layout["thinking"].update(
                    safe_create_panel(thinking, "AI Thinking", "cyan")
                )

                # Update decisions
                if decision:
                    layout["decisions"].update(
                        safe_create_panel(decision, "Latest Decision", "yellow")
                    )
                ...
                if "Executing" in thinking:
                    ...
                    layout["tools"].update(
                        safe_create_panel(tools_content, "Tool Execution", "green")
                    )

                # Update results
                results_content = f"⏱️ Step: {i+1}/{len(workflow)}\n🔧 Tools: {len(tools_executed)}\n📊 Progress: {(i+1)/len(workflow)*100:.0f}%"
                layout["results"].update(
                    safe_create_panel(results_content, "Live Results", "magenta")
                )
