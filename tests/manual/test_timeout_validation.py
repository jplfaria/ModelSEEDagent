...
async def test_gpto1_timeout():
    ...
    assert (
        timeout_seconds == 120
    ), f"Expected 120s timeout for {model_name}, got {timeout_seconds}s"
    assert (
        gpto1_llm._timeout == 120.0
    ), f"Expected 120s LLM timeout, got {gpto1_llm._timeout}s"
...
async def test_standard_model_timeout():
    ...
    assert (
        timeout_seconds == 30
    ), f"Expected 30s timeout for {model_name}, got {timeout_seconds}s"
    assert (
        standard_llm._timeout == 30.0
    ), f"Expected 30s LLM timeout, got {standard_llm._timeout}s"
