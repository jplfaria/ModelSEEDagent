...
def test_argo_llm_timeout_configuration():
    ...
    assert (
        o1_llm._timeout == 120.0
    ), f"Expected 120s timeout for o1 model, got {o1_llm._timeout}s"
    ...
    assert (
        standard_llm._timeout == 30.0
    ), f"Expected 30s timeout for standard model, got {standard_llm._timeout}s"
...
def test_timeout_logic_manually():
    ...
    for model_name, expected_is_o1, expected_timeout in test_cases:
        ...
        logger.info(
            f"🧪 Model '{model_name}' -> is_o1: {is_o1_model}, timeout: {timeout_seconds}s"
        )

        assert (
            is_o1_model == expected_is_o1
        ), f"Model '{model_name}' o1 detection failed"
        assert (
            timeout_seconds == expected_timeout
        ), f"Model '{model_name}' timeout incorrect"
...
async def test_signal_timeout_mechanism():
    ...
    try:
        ...
        assert (
            0.9 <= duration <= 1.5
        ), f"Signal timeout duration {duration:.2f}s not in expected range"
    finally:
