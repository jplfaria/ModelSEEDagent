        ...
        try:
            ...
            if hasattr(result, "metadata") and "audit_file" in result.metadata:
                ...
                if os.path.exists(audit_file):
                    ...
                    print(
                        f"   • Success: {audit_data.get('output', {}).get('success')}"
                    )
                    print(
                        f"   • Execution time: {audit_data.get('execution', {}).get('duration_seconds')}s"
                    )
                    ...
                    print(
                        f"   • Console output captured: {len(stdout)} stdout chars, {len(stderr)} stderr chars"
                    )
                    ...
                    print(
                        f"   Output keys: {list(audit_data.get('output', {}).keys())}"
                    )
                    print(
                        f"   Execution keys: {list(audit_data.get('execution', {}).keys())}"
                    )

                else:
                    print(
                        f"❌ Audit file path provided but file not found: {audit_file}"
                    )

            else:
                print("❌ No audit file metadata found in tool result")
                print(
                    "This suggests the audit system is not properly intercepting tool execution"
                )
    ...
    if logs_dir.exists():
        ...
        for session_dir in session_dirs[:3]:  # Check first 3 sessions
            ...
            if audit_dir.exists():
                ...
                print(
                    f"   • Session {session_dir.name}: {len(audit_files)} audit files"
                )
                ...
                if audit_files:
                    ...
                    try:
                        ...
                        print(
                            f"     - Sample audit from {sample_file.name}: tool '{sample_data.get('tool_name')}' at {sample_data.get('timestamp')}"
                        )
                    except:
