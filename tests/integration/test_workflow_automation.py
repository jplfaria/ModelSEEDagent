...
async def test_workflow_engine():
    ...
    try:
        ...
        console.print(
            f"[{status_color}]{'✅' if result.success else '❌'} Workflow {result.status.value}: {result.message}[/{status_color}]"
        )
        console.print(
            f"[cyan]📊 Duration: {result.duration:.1f}s, Steps: {result.completed_steps}/{result.total_steps}[/cyan]"
        )

        # Display performance metrics
        metrics = engine.get_performance_metrics()
        console.print(
            f"[yellow]📈 Engine Metrics: {metrics['total_executions']} executions, {metrics['success_rate']:.1%} success rate[/yellow]"
        )
...
async def test_batch_processor():
    ...
    try:
        from src.workflow.batch_processor import BatchProcessingStrategy, BatchProcessor
        from src.workflow.workflow_definition import (
            StepType,
            WorkflowDefinition,
            WorkflowStep,
        ...
        for strategy in strategies:
            ...
            status_color = "green" if result.success else "red"
            console.print(
                f"[{status_color}]{'✅' if result.success else '❌'} Batch {result.status.value}: {result.message}[/{status_color}]"
            )
            console.print(
                f"[cyan]📊 Duration: {result.duration:.1f}s, Jobs: {result.successful_jobs}/{result.total_jobs}[/cyan]"
            )
        ...
        console.print(
            f"[green]✅ Model batch completed: {result.successful_jobs}/{result.total_jobs} models processed[/green]"
        )

        # Display performance metrics
        metrics = processor.get_performance_metrics()
        console.print(
            f"[yellow]📈 Batch Metrics: {metrics['total_batches']} batches, {metrics['batch_success_rate']:.1%} success rate[/yellow]"
        )
...
def test_template_library():
    ...
    try:
        ...
        if basic_template:
            ...
            workflow = library.create_workflow_from_template(
                "basic_analysis", parameters
            )
            console.print(
...
async def test_advanced_scheduler():
    ...
    try:
        ...
        from src.workflow.scheduler import (
            AdvancedScheduler,
            SchedulingStrategy,
            TaskPriority,
        )
        ...
        scheduler = AdvancedScheduler(
            max_concurrent_workflows=2, strategy=SchedulingStrategy.PRIORITY
        )
        ...
        for workflow, priority in workflows:
            task_id = scheduler.schedule_workflow(workflow=workflow, priority=priority)
        ...
        for i in range(8):
            ...
            completed_count = len(
                [
                    t
                    for t in task_ids + [delayed_task_id] + recurring_task_ids
                    if scheduler.get_task_status(t)
                    and scheduler.get_task_status(t).status.value
                    in ["completed", "failed"]
                ]
            )
            ...
            console.print(
                f"[cyan]📈 Progress: {completed_count}/{total_tasks} tasks processed[/cyan]"
            )
        ...
        stats_table.add_row(
            "Average Execution Time", f"{stats.average_execution_time:.1f}s"
        )
        stats_table.add_row("Throughput", f"{stats.throughput:.1f} tasks/hour")

        console.print(
            Panel(stats_table, title="📊 Scheduler Performance", border_style="green")
        )
...
async def test_integration_scenario():
    ...
    try:
        ...
        console.print(
            "[green]✅ Integration scenario would combine all components[/green]"
        )
        console.print(
            "[cyan]📋 Templates → Workflows → Scheduling → Batch Processing → Monitoring[/cyan]"
        )
...
async def main():
    ...
    console.print("\n" + "=" * 80)
    console.print(
        Panel("[bold blue]📋 Test Results Summary[/bold blue]", border_style="blue")
    )
    ...
    for test_name, success in results:
        if success:
            results_table.add_row(
                test_name, "[green]✅ PASSED[/green]", "[green]Success[/green]"
            )
            passed += 1
        else:
            results_table.add_row(
                test_name, "[red]❌ FAILED[/red]", "[red]Failed[/red]"
            )
    ...
    if passed == total:
        ...
        console.print(
            "[green]✅ Phase 3.3: Advanced Workflow Automation is ready for production![/green]"
        )
    else:
        console.print(
        ...
        console.print(
            "[yellow]💡 Review failed components and fix issues before deployment[/yellow]"
        )
