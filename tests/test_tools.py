...
class TestFBATool:
    def test_init(self):
        config = {
            ...
            "fba_config": {"default_objective": "BIOMASS", "solver": "glpk"},
        }
        tool = FBATool(config)
    ...
    def test_run_fba(self, test_model_path):
        config = {
            "name": "fba_tool",
            "description": "FBA tool",
            "fba_config": {"default_objective": "R1", "solver": "glpk"},
        }
        tool = FBATool(config)
...
class TestModelAnalysisTool:
    def test_init(self):
        config = {
            ...
            "analysis_config": {"flux_threshold": 1e-6},
        }
    ...
    def test_model_analysis(self, test_model_path):
        tool = ModelAnalysisTool(
            {"name": "analysis_tool", "description": "Analysis tool"}
        )
        result = tool._run(test_model_path)
...
class TestPathwayAnalysisTool:
    def test_pathway_analysis(self, test_model_path):
        tool = PathwayAnalysisTool(
            {"name": "pathway_tool", "description": "Pathway tool"}
        )
        result = tool._run(
            {
        ...
        if result.success:
            ...
            assert (
                ...
                or "pathway" in result.error.lower()
            )
...
class TestToolRegistry:
    ...
    def test_create_tool(self):
        tool = ToolRegistry.create_tool(
            "run_metabolic_fba", {"name": "fba", "description": "FBA tool"}
        )
