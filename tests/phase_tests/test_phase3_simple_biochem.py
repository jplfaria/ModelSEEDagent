    ...
    for query in search_queries:
        try:
            ...
            if total_results > 0:
                ...
                if alias_results:
                    print(
                        f"   Alias match: {alias_results[0][1]} ({alias_results[0][0]}) via {alias_results[0][3]}"
                    )
                success_count += 1
            else:
...
def test_performance():
    ...
    try:
        ...
        print(
            f"✅ Lookup performance: {len(test_ids)} queries in {duration:.3f} seconds"
        )
        ...
        print(
            f"✅ Search performance: Found {len(results)} glucose matches in {end_time - start_time:.3f} seconds"
        )
