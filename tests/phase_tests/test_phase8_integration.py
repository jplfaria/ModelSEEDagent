...
class IntegrationTestSuite:
    ...
    async def _test_workflow_integration(self):
        ...
        try:
            ...
            print(
                f"✅ CLI Integration: {processed_commands} commands processed in {cli_time:.3f}s"
            )
            ...
            print(
                f"✅ Agent Factory Integration: {len(created_agents)} agents created in {factory_time:.3f}s"
            )
            ...
            print(
                f"✅ Tool Registry Integration: {len(available_tools)} tools available in {tools_time:.3f}s"
            )
    ...
    async def _test_performance_optimization(self):
        ...
        try:
            ...
            for i in range(10):
                ...
                # Check cache first
                cached_result = self.performance_optimizer.reasoning_cache.get(
                    cache_key
                )
            ...
            avg_hit_time = sum(
                test["duration_ms"] for test in cache_tests if test["cache_hit"]
            ) / max(cache_hits, 1)
            avg_miss_time = sum(
                test["duration_ms"] for test in cache_tests if not test["cache_hit"]
            ) / max(len(cache_tests) - cache_hits, 1)

            print(f"✅ Cache Performance: {cache_hits}/{len(cache_tests)} hits")
            print(
                f"   Hit time: {avg_hit_time:.1f}ms, Miss time: {avg_miss_time:.1f}ms"
            )
        ...
        try:
            ...
            for i, step in enumerate(steps):
                ...
                if step.tool_selected == "run_metabolic_fba":
                    ...
                    mock_result = {
                        "essential_nutrients": 14,
                        "medium_complexity": "moderate",
                    }
                ...
                print(
                    f"   Step {i+1}: {step.tool_selected} → {step_duration*1000:.1f}ms"
                )
            ...
            avg_step_time = sum(
                result["duration_ms"] for result in execution_results
            ) / len(execution_results)

            print(
                f"✅ Chain Execution: {successful_steps}/{len(steps)} steps successful"
            )
            ...
            nutrition_step_executed = any(
                r["tool"] == "find_minimal_media" for r in execution_results
            )

            adaptive_reasoning_works = growth_rate > 0.8 and nutrition_step_executed

            print(
                f"✅ Adaptive Reasoning: {'Working' if adaptive_reasoning_works else 'Needs improvement'}"
            )
            print(
                f"   Growth rate {growth_rate} → Nutrition analysis: {nutrition_step_executed}"
            )
        ...
        try:
            ...
            for obs in observations:
                ...
                generated_hypotheses.append(
                    {"observation": obs, "hypothesis": hypothesis}
                )

            print(
                f"✅ Hypothesis Generation: {len(generated_hypotheses)} hypotheses from {len(observations)} observations"
            )
            ...
            for hyp_data in generated_hypotheses:
                ...
                # Evaluate hypothesis
                supporting_evidence = [
                    e for e in evidence_collected if e["supports_hypothesis"]
                ]
                avg_evidence_strength = sum(
                    e["strength"] for e in supporting_evidence
                ) / len(supporting_evidence)

                hypothesis_supported = (
                    len(supporting_evidence) >= len(evidence_collected) * 0.5
                )
                ...
                print(
                    f"   {hypothesis['type']}: {status} (strength: {avg_evidence_strength:.2f})"
                )

            supported_hypotheses = sum(
                1 for result in test_results if result["hypothesis_supported"]
            )
            avg_test_time = sum(
                result["test_duration_ms"] for result in test_results
            ) / len(test_results)

            print(
                f"✅ Hypothesis Testing: {supported_hypotheses}/{len(test_results)} hypotheses supported"
            )
        ...
        try:
            ...
            for scenario in collaboration_scenarios:
                ...
                if triggers_collaboration:
                    ...
                    human_rationale = (
                        f"Agreed with AI recommendation for {selected_option}"
                    )
                    ...
                    decision = {
                        ...
                        "agreement_with_ai": selected_option
                ...
                decision_type = (
                    "Collaborative" if triggers_collaboration else "Independent"
                )
                agreement = "Yes" if decision.get("agreement_with_ai", True) else "No"
                print(
                    f"   {scenario['context'][:40]}... → {decision_type} (Agreement: {agreement})"
                )

            # Analyze collaboration effectiveness
            collaborative_decisions = [
                r
                for r in collaboration_results
                if not r.get("independent_decision", False)
            ]
            ai_human_agreements = sum(
                1 for r in collaborative_decisions if r.get("agreement_with_ai", False)
            )
            avg_collaboration_time = sum(
                r["collaboration_time_ms"] for r in collaborative_decisions
            ) / max(len(collaborative_decisions), 1)
            avg_confidence = sum(
                r["confidence_score"] for r in collaboration_results
            ) / len(collaboration_results)

            print(
                f"✅ Collaborative Decisions: {len(collaborative_decisions)}/{len(collaboration_results)}"
            )
            print(
                f"   AI-Human Agreement: {ai_human_agreements}/{len(collaborative_decisions)}"
            )
            ...
            self.test_results["collaborative_workflow"] = {
                ...
                "ai_human_agreement_rate": ai_human_agreements
                / max(len(collaborative_decisions), 1),
        ...
        try:
            ...
            for exp in experiences:
                if exp["success"]:
                    ...
                    tool_sequences[seq_key][
                        "success_rate"
                    ] = 1.0  # All successful in mock data
            ...
            for exp in experiences:
                query_type = (
                    "comprehensive" if "comprehensive" in exp["query"] else "specific"
                )
            ...
            print(
                f"✅ Pattern Extraction: {len(extracted_patterns)} patterns identified"
            )
            ...
            for query in test_queries:
                ...
                for pattern in extracted_patterns:
                    ...
                    if (
                        ...
                        and "comprehensive" in pattern["description"].lower()
                    ):
                        relevance_score += 0.6
                    if (
                    ...
                    if relevance_score > 0:
                        matching_patterns.append(
                            {"pattern": pattern, "relevance": relevance_score}
                        )
                ...
                print(
                    f"   '{query[:30]}...' → {recommendation['matched_pattern'][:40]}..."
                )

            # Calculate learning effectiveness
            successful_matches = sum(
                1 for app in pattern_applications if app["relevance_score"] > 0.5
            )
            avg_confidence = sum(
                app["confidence"] for app in pattern_applications
            ) / len(pattern_applications)

            print(
                f"✅ Pattern Application: {successful_matches}/{len(pattern_applications)} successful matches"
            )
            print(f"   Average confidence: {avg_confidence:.2f}")
    ...
    async def _test_end_to_end_scenarios(self):
        ...
        try:
            ...
            _ = "Perform complete metabolic characterization of E. coli model"
                ...
                "steps": [
                    {
                        ...
                        "reasoning": "Baseline growth assessment",
                    },
                    {
                        "tool": "find_minimal_media",
                        "reasoning": "Nutritional requirements",
                    },
                    {
                        "tool": "analyze_essentiality",
                        "reasoning": "Essential components",
                    },
                ],
            ...
            for step in reasoning_chain["steps"]:
                ...
                if step["tool"] == "run_metabolic_fba":
                    ...
                    result = {
                        "essential_nutrients": 14,
                        "nutrient_types": ["carbon", "nitrogen", "phosphorus"],
                    }
            ...
            _ = "Model shows unexpectedly low growth in minimal medium"
            ...
            test_results = [
                {
                    "tool": "identify_auxotrophies",
                    "result": {"auxotrophies": ["histidine", "methionine"]},
                    "supports": True,
                },
                {
                    "tool": "find_minimal_media",
                    "result": {"complex_requirements": True},
                    "supports": True,
                },
            ]
            ...
            print(
                f"   Evidence strength: {hypothesis_conclusion['evidence_strength']:.2f}"
            )
            ...
            self.test_results["end_to_end_scenarios"] = {
                ...
                "scenario2_results": {
                    "hypothesis_supported": hypothesis_conclusion["status"]
                    == "SUPPORTED",
    ...
    def _generate_integration_report(self):
        ...
        for test_name, result in self.test_results.items():
            ...
            duration = result.get(
                "total_duration_ms", result.get("avg_scenario_duration_ms", 0)
            )
            ...
            if status == "PASS":
                ...
                if "steps_executed" in result:
                    print(
                        f"   Steps executed: {result['steps_executed']}/{result.get('total_steps', result['steps_executed'])}"
                    )
                ...
                if "hypotheses_supported" in result:
                    print(
                        f"   Hypotheses supported: {result['hypotheses_supported']}/{result['hypotheses_tested']}"
                    )
                if "ai_human_agreement_rate" in result:
                    print(
                        f"   AI-Human agreement: {result['ai_human_agreement_rate']:.1%}"
                    )
                if "successful_matches" in result:
                    print(
                        f"   Pattern matches: {result['successful_matches']}/{result['test_queries']}"
                    )
        ...
        if "reasoning_chain_execution" in self.test_results:
            ...
            if reasoning["status"] == "PASS":
                print(
                    f"   Chain execution: {reasoning['avg_step_duration_ms']:.1f}ms per step"
                )
                print(
                    f"   Adaptive reasoning: {'✅' if reasoning['adaptive_reasoning'] else '❌'}"
                )
        ...
        if "hypothesis_workflow" in self.test_results:
            ...
            if hyp["status"] == "PASS":
                print(
                    f"   Hypothesis success rate: {hyp['hypotheses_supported']}/{hyp['hypotheses_tested']}"
                )
