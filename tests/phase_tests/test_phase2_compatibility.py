...
def test_compatibility_tool_basic():
    ...
    try:
        ...
        print(
            f"✅ Loaded test model: {len(model.reactions)} reactions, {len(model.metabolites)} metabolites"
        )
        ...
        # Test compatibility
        result = tool._run({"modelseed_model": model, "media_condition": "Complete"})
...
def test_sbml_roundtrip():
    ...
    try:
        ...
        # Compare structures
        print(
            f"   Original: {len(original_model.reactions)} rxns, {len(original_model.metabolites)} mets"
        )
        print(
            f"   COBRApy:  {len(cobra_model.reactions)} rxns, {len(cobra_model.metabolites)} mets"
        )
        ...
        print(
            f"   Growth rates - Original: {orig_growth:.6f}, COBRApy: {cobra_growth:.6f}"
        )
        ...
        print(
            f"   Round-trip {'✅ PASSED' if success else '❌ FAILED'} (tolerance: {tolerance})"
        )
...
def test_convenience_function():
    ...
    try:
        ...
        print(
            f"   Compatibility metrics available: {len(result['compatibility_metrics'])} metrics"
        )
